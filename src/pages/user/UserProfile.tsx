...
const UserProfile = () => {
    // Update the user selector to use the User type
    const user = useSelector((state: RootState) => state.user.user) as User;
    const [activeTab, setActiveTab] = useState('overview');

    const profile = {
        name: user?.fullName || "John Doe",
        title: user?.title || "Agricultural Manager",
        location: user?.location || "Central Valley, CA",
        experience: user?.experience || "15+ years",
        email: user?.email || "john.doe@example.com",
        bio: user?.bio || "I'm a passionate agricultural manager with a love for farming and technology.",
        avatar: user?.avatar || "/api/placeholder/128/128",
        coverimage: user?.coverImage || "/api/placeholder/1400/400",
        farmSize: user?.farmSize || "250 acres",
        primaryCrops: user?.primaryCrops || ["Corn", "Soybeans", "Wheat", "Alfalfa"],
        certifications: user?.certifications || ["Organic Certified", "Sustainable Farming", "GAP Certified"],
        lastSoilTest: user?.lastSoilTest || "2024-03-15",
        activeSubscription: user?.activeSubscription || "Premium",
        farmStats: {
            soilHealth: user?.soilHealth || 85,
            waterEfficiency: user?.waterEfficiency || 92,
            yieldForecast: user?.yieldForecast || 78,
            sustainability: user?.sustainability || 88
        },
    ...
    return (
        <div className="max-w-7xl mx-auto bg-gray-50 min-h-screen">
            ...
            <ProfileHero coverImage={profile.coverimage} />
                                            ...
                                            <div key={item.name} className="flex items-center justify-between p-4 hover:bg-gray-50">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-sm text-gray-500">Last maintained: {item.lastMaintenance}</p>
                                                    </div>
                                                </div>
